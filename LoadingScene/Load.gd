...
var texts = ["Java is a versatile, object-oriented programming language known for its 
platform independence, robustness, and extensive standard library",
"A flowchart in the context of Java typically refers to a visual representation of the 
logical flow of a Java program. It's a diagrammatic representation that illustrates the 
steps or sequence of operations involved in executing a Java program.", "Flowcharts serve 
as a helpful tool for planning, understanding, and communicating the structure and flow of 
a Java program."];
...
func _process(_delta):
	...
	if ResourceLoader.load_threaded_get_status(next_scene_path) == ResourceLoader.THREAD_LOAD_LOADED: # Checks to see if the file is finished loading
		...
		await get_tree().create_timer(3).timeout # Added a delay for testing purposes. Can be removed in the final game.
